...
# # App config
# # ======================
# st.set_page_config(page_title="Road Safety – Game | Explore | Journey", page_icon="🛣️", layout="wide")
...
#                     f'{b["num_lanes"]}L', f'curv {b["curvature"]:.1f}',
#                     f'{b["speed_limit"]}km/h',
#                     ["Day","Twilight","Dark"][b["lighting"]],
#                     ["Clear","Rain","Fog"][b["weather"]],
#                 ])
#                 # Fix bracket above
#             # Correct description build
#             tbl = []
#             for s in best_steps:
#                 b = s["Base"]
#                 desc = " ".join([
...
# ======================
# App config - ONLY ONCE, FIRST COMMAND
# ======================
st.set_page_config(page_title="Road Safety – Game | Explore | Journey", page_icon="🛣️", layout="wide")

# ======================
# Styling - AFTER page config
# ======================
st.markdown("""
<style>
:root {
...
with tab_journey:
    ...
    if "j_thr" not in st.session_state: st.session_state.j_thr = 0.60
    if "j_seed" not in st.session_state: st.session_state.j_seed = 2025
    if "j_hist" not in st.session_state: st.session_state.j_hist = []
    if "j_alive" not in st.session_state: st.session_state.j_alive = True
    if "j_opts" not in st.session_state: st.session_state.j_opts = None
    if "j_opt_count" not in st.session_state: st.session_state.j_opt_count = 3
    if "j_all_opts" not in st.session_state: st.session_state.j_all_opts = {}

    st.subheader("Journey (start → finish)")
    st.caption("Pick the safest option at each checkpoint. Risks are hidden until the end. If your chosen option's risk ≥ threshold, the journey ends immediately.")
